...
test('Mercury and Venus combust for Darbhanga chart', async () => {
  const { computePositions } = await astro;
  ...
  assert.ok(vLabel.includes('(C)'), 'Venus label should include (C)');
});
...
test('combust planets show (C) in chart summary', async () => {
  ...
  const hasVenusCombust = rows.some((r) => r.startsWith('Ve') && r.includes('(C)'));
  assert.ok(hasVenusCombust, 'summary should include Ve with (C)');
});
